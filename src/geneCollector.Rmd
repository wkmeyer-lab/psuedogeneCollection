...
mergeData <- function(sg, genes){
  ...
  #reformatting genes and species_gene to prepare or merging
  df_sg <- data.frame(genesOfKey = sg[[1]][[1]][1:5])
  names(df_sg)[1] <- names(sg)[1]
  
  for (i in 2:3){ #nrows(species_gene)
    df_sg[[i]] <- sg[[i]][[1]]
    names(df_sg)[i] <- names(sg)[i]
  }
   
  df_genes <- data.frame(species = genes[1:15])
  
    #data now is formatted correctly, ready to merge
  for(i in 1:3){ #ncol(df_sg)
    geneAlignment <- merge(df_genes, df_sg, by.x = "species", by.y = names(df_sg)[i], all.x = TRUE)
  }
    View(geneAlignment)
    
    #Merged Data Frames. Now must append 1 or 0 values 
  matchIndex <- list()
  for(i in 1:3){ # ncol(df_sg)
    lookup <- names(sg)[i] #contains all genes: name --> tibble --> first vector
    matchIndex <- match(geneAlignment, lookup)
  }
    print(matchIndex)
}
